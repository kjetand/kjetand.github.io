...
You guessed it, we use an enum type for this purpose.
Not a regular _C-enum_, that's simply _ugly_ and wrong!
C++ `enum class` is a much better choice.
Why you ask?
Because an `enum class` is a _strong type_ meaning that it cannot be implicitly converted to e.g. integer types, unlike C-enums.
